...
def train_one_fold(x, y, train_idx, valid_idx, x_test, args, seed, fold, device):
    ...
    best_state = None
    stale = 0
    swa_start = total_epochs // 3  # start SWA after 1/3 of training
    ...
    # Apply SWA model for final prediction
...
def maybe_blend(out_dir, y, test_ids, dl_oof, dl_test):
    ...
    for w_dl in [0.05, 0.10, 0.15, 0.20, best["w_dl"]]:
        ...
        save_submission(test_ids, prob_test, out_dir / f"submission_v20_dl_v34_w{w_dl:.2f}.csv")
    return best
...
def main():
    ...
    parser.add_argument("--out-dir", default="dl_output_v34")
    ...
    np.save(out_dir / "dl_v34_oof.npy", dl_oof)
    np.save(out_dir / "dl_v34_test.npy", dl_test)
    pd.DataFrame({"PassengerId": train_df["PassengerId"], "Transported": y, "prob": dl_oof}).to_csv(
        out_dir / "dl_v34_oof.csv", index=False
    )
    pd.DataFrame({"PassengerId": test_ids, "prob": dl_test}).to_csv(
        out_dir / "dl_v34_test.csv", index=False
    )
    save_submission(test_ids, dl_test, out_dir / "submission_dl_v34.csv")
    ...
    (out_dir / "dl_v34_report.json").write_text(json.dumps(report, indent=2), encoding="utf-8")
